...
namespace chun
{
    ...
    public class NaviManager : MonoBehaviour
    {
        ...
        private bool firstDoorOpen = true;
        ...
        void Update()
        {
            ...
            if (tresurebox.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("box_open"))
            {
                if (firstDoorOpen)
                {
                    firstDoorOpen = false;
            ...
            }else
            {
                ...
                if (button && time > 60 && !pictureburnfinish)
                {
                    button = false;
                    time = 0;
        ...
        {
            navi.GoTo(dooraxis.transform.position+new Vector3(-1,1,-2));
            yield return new WaitForSeconds(1.0f);
            navi.Say("これでこの部屋から出られるね!",true);
            yield return null;
        ...
        IEnumerator FirstIntro()
        {
            ...
            navi.GoTo(paper.transform.position + new Vector3(-1, 0, 0.6f));
            yield return new WaitForSeconds(3.0f);
